 ...
 *** Last Motification: Feb 5th
 *** The modified code: in line 193, line 220, line 238, line 242, line 257
 ***/
#include <stdio.h>
#include <stdlib.h>
                    ...
                    float *coord2)   /* [numdims] */
{
    ...
    for (i=0; i<numdims; i++)
        ans = ans + (coord1[i] - coord2[i]) * (coord1[i] - coord2[i]);
                   ...
                   int    *membership)        /* out: [numObjs] */
{
    ...
    float mythreshold = threshold * numObjs;
    float temp;
    ...
    #pragma omp parallel for private(i)   // this will be fast
    ...
    do {
        ...
        if (is_perform_atomic) {
            ...
            for (i=0; i<numObjs; i++) {
                ...
                if (membership[i] != index) delta = delta + 1.0;  // change for delta += 1.0
                    ...
                    shared(objects,clusters,membership,local_newClusters,local_newClusterSize)
            {
                ...
                /****
                 **** Motification: Add nowait to speed up
                 ****/
                ...
                for (i=0; i<numObjs; i++) {
                    ...
                    /* if membership changes, increase delta by 1 */
                    if (membership[i] != index) delta = delta + 1.0; //change from delta += 1.0
            ...
            /**** Motification: The order of the 2 for loops are exchanged.
             **** In this way, data in newClusterSize and local_newClusterSize are read and written faster
             ****/
        ...
        /**** Motification1: The order of the 2 for loops are exchanged.
         **** In this way, data in newClusterSize, clusters and newClusters are read and written faster
         ****/
        
        /****
         **** Motification2: To do less division, we changed clusters[i][j] = newClusters[i][j] / newClusterSize[i]
         **** temp = 1.0 / newClusterSize[i]  clusters[i][j] = newClusters[i][j] * temp
         ****/
        for (j=0; j<numCoords; j++) {
            for (i=0; i<numClusters; i++) {
                if (newClusterSize[i] > 1){
                    temp = 1.0 / newClusterSize[i];
      ...
      /**** Motification:  To do less division, we changed "delta /= numObjs";  to mythreshold = numObjs * threshold
       **** And instead of using delta > threshold, we use delta > mythreshold
       ****/
    } while (delta > mythreshold && loop++ < 500);
